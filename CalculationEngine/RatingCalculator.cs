...
namespace RankingLibrary.CalculationEngine
{
    public class RatingCalculator : CalculateMaster
    {
        ...
        public RatingCalculator(double newTau, double volatility) : base()
        {
            tau = newTau;
        ...
        public void UpdateRankings(RatingPeriodResults results)
        {
            foreach(Rating player in results.GetParticipants())
            {
                ...
                else
                {
                    ...
                    player.WorkingRatingDeviation = CalculateNewRatingDeviation(player.GetGlicko2RatingDeviation(), player.Volatility);
        ...
        public void CalculateNewRating(Rating player, IList<Result> results)
        {
            ...
            player.WorkingRating = player.GetGlicko2Rating()+ (Math.Pow(newPhi, 2) * OutcomeBasedRating(player, results));
        ...
        public static double F(double x, double delta, double phi, double v, double a, double tau)
        {
        ...
        /// <returns></returns>
        public static double E(double playerRating, double opponentRating, double opponentDeviation)
        {
        ...
        /// <param name="results"></param>
        /// <returns></returns>
        public static double V(Rating player, IEnumerable<Result> results)
        {
            double v = 0.0;
        ...
        public double Delta(Rating player, IList<Result> results)
        {
        ...
        {
            throw new NotImplementedException();// Glicko requires more than 1 result 
        }
        ...
        public double ConcertRatingDeviationToGlicko2Scale(double ratingDeviation)
        {
            return (ratingDeviation / MULTIPLIER);
        }

        public override double GetDefaultRating()
        {
